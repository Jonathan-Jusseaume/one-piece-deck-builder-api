...
public class DeckSpecification {
        ...
        return (
            (root, criteriaQuery, criteriaBuilder) ->
                criteriaBuilder.equal(
                    root.get(DeckEntity_.USER).get(UserEntity_.MAIL),
                    mail
                )
        );
    }
                        ...
                        predicateWord = criteriaBuilder.and(
                            criteriaBuilder.or(
                                criteriaBuilder.isNull(
                                    root.get(DeckEntity_.DESCRIPTION)
                                ),
                                criteriaBuilder.not(
    ...
    public static Specification<DeckEntity> byColor(Set<Color> colors) {
        return (
            (root, criteriaQuery, criteriaBuilder) -> {
                Join<DeckEntity, CardEntity> leaderJoin = root.join(
                    DeckEntity_.LEADER
                );
                Join<CardEntity, Color> colorsJoin = leaderJoin.join(
                    CardEntity_.COLORS
                );
    ...
    public static Specification<DeckEntity> byUserFavoriteDeck(String mail) {
        return (
            (root, criteriaQuery, criteriaBuilder) -> {
                SetJoin<DeckEntity, UserEntity> join = root.joinSet(
                    DeckEntity_.USERS_FAVORITE
                );
