...
public class CardService {
    ...
    private Specification<CardEntity> convertCardSearchToSpecification(
        CardSearch cardSearch
    ) {
        ...
        addToFilter(
            builder,
            cardSearch.getColors(),
            CardSpecification::byColor
        );
        addToFilter(
            builder,
            cardSearch.getTagsId(),
            CardSpecification::byTagId
        );
        addToFilter(
            builder,
            cardSearch.getRarities(),
            CardSpecification::byRarity
        );
        addToFilter(
            builder,
            cardSearch.getProductsId(),
            CardSpecification::byProductId
        );
        addToFilter(builder, cardSearch.getCosts(), CardSpecification::byCost);
        addToFilter(
            builder,
            cardSearch.getPowers(),
            CardSpecification::byPower
        );
        addKeywordToFilter(builder, cardSearch.getKeyword());
        return builder.build();
    }

    private <T> void addToFilter(
        SpecificationBuilder<CardEntity> builder,
        Set<T> criteria,
        Function<Set<T>, Specification<CardEntity>> specFunction
    ) {
